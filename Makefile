...
COMMAND=npx $(GRPC_TOOL) --plugin=${TYPESCRIPT_PLUGIN} --js_out=import_style=commonjs,binary:$(OUTPUT) --grpc_out=grpc_js:$(OUTPUT) --ts_out=grpc_js:$(OUTPUT) --proto_path=$(GOOGLE_PROTO_PATH) --proto_path=$(WAGE_PROTO_PATH) $(WAGE_PROTO_PATH)/jp/wage/api/v1/*.proto $(GOOGLE_PROTO_PATH)/google/type/date.proto $(GOOGLE_PROTO_PATH)/google/rpc/error_details.proto 
